...
class WikipediaSpider(Spider):
    ...
    def parse(self, response: TextResponse):
        ...
        for box_selector in boxes_selectors.css("#main-dyk, #main-cur, #main-tfa, #main-tga"):
            for message in self.parse_box(response, box_selector):
                yield message

    def parse_box(self, response: TextResponse, box_selector: Selector):
    ...
    def parse_article_box(self, response: TextResponse, box_selector: Selector) -> MessageItem:
        message_loader = MessageLoader(item=MessageItem(), selector=box_selector, response=response)
        message_loader.add_xpath('text', './p')
        message_loader.add_xpath('image', './/a/img')
        message_loader.add_value('url', response.url)
        yield message_loader.load_item()

    def parse_list_box(self, response: TextResponse, box_selector: Selector) -> MessageItem:
        message_loader = MessageLoader(item=MessageItem(), selector=box_selector, response=response)
